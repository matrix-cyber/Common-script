...
import TestBase.ActionBase.M3_NetworkSettings.M3_2_WanSettings.ADSL_by_Lan as ACTION
import TestBase.ActionBase.Base_Action.SSH_Action as SA
import TestBase.DataBase as TD
import inspect
import time

class ADSL_by_Lan: #基于物理网卡的多拨
    ...
    def __init__(self, web_handler, ssh_handler, **TestData): #模块初始化
        ...
        self.ElementCss = CSS.ElementBase.Adsl_by_Lan#模块元素库
        self.TestData = TestData #测试数据
        self.LoginData = TD.TestData.Login
        self.ACTION = ACTION.ADSL_by_Lan(self.web_handler,self.ssh_handler, self.TestData)
    ...
    def TestMethod1(self):  # 测试添加和认证
        ...
        time.sleep(1)
        self.find_element(self.ElementCss["基于物理网卡的多拨"]).click()
        time.sleep(1)
    ...
    def run_Method(self):
        i = 0
        for Fname, Func in ADSL_by_Lan.__dict__.items():
    ...
    def run_tests(self):
        TestData_all = TD.TestData.Adsl_by_Lan
        DataLen = len(TestData_all)
        RunTestNum = 0
        for i in range(0, DataLen):
            if TestData_all[i]["是否执行"] == "Y":
                print(TestData_all[i]["测试例"])
                NEW_RUN = ADSL_by_Lan(self.web_handler, self.ssh_handler, **TestData_all[i])
